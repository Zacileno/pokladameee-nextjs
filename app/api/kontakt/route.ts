...
export async function POST(req: NextRequest) {
  ...
  if (isPrihlaska) {
    ...
    await resend.emails.send({
      from: 'no-reply@pokladameee.cz',
      to: email,
      subject: 'Přijali jsme tvoji přihlášku — pokládámeee.cz',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px;">
          <div style="background: #154C86; padding: 24px 32px; border-radius: 8px 8px 0 0;">
          ...
          <div style="background: #f8f7f5; padding: 32px; border-radius: 0 0 8px 8px; border: 1px solid #efefed; border-top: none;">
            <h2 style="color: #000; font-size: 20px; margin: 0 0 16px;">Díky za přihlášku, ${jmeno}!</h2>
            <p style="color: #3d3d3a; line-height: 1.7; margin: 0 0 16px;">Tvoji přihlášku jsme dostali${pozice ? ` na pozici <strong>${pozice}</strong>` : ''}. Ozveme se ti co nejdříve — obvykle do pár pracovních dní.</p>
            <p style="color: #3d3d3a; line-height: 1.7; margin: 0 0 24px;">Mezitím se na nás klidně podívej na webu nebo nám zavolej.</p>
