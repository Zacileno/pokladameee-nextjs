...
export default function KontaktSekce() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    // TODO: Formspree ID doplnit
    // const res = await fetch('https://formspree.io/f/XXXXXXXX', { method: 'POST', ... })
    await new Promise(r => setTimeout(r, 800)) // simulace
    setSent(true)
    setLoading(false)
  }
  ...
  return (
    <section id="kontakt" className="section" style={{ background: 'white' }}>
          ...
          <div>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ color: 'var(--gray-700)', fontSize: 14, marginTop: 6, fontStyle: 'italic' }}>
                  „Každou podlahu řešíme osobně. Přijedeme se podívat a řekneme vám rovnou, co čekat."
                </div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={label} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    ...
                    {href ? (
                      <a href={href} style={{ fontWeight: 700, color: 'var(--blue)', fontSize: 16 }}>{val}</a>
                    ) : (
          ...
          <div style={{ background: 'var(--gray-50)', borderRadius: 16, padding: '40px 36px', border: '1px solid var(--gray-100)' }}>
            {sent ? (
              <div style={{ textAlign: 'center', padding: '48px 0' }}>
                ...
                <p style={{ color: 'var(--gray-400)', fontSize: 16 }}>Ozveme se vám do 24 hodin.<br />Těšíme se na spolupráci.</p>
              </div>
            ) : (
      ...
      <style>{`@media(max-width:900px){.kontakt-grid{grid-template-columns:1fr!important}}`}</style>
    </section>
  )
}

const inp: React.CSSProperties = {
  padding: '11px 14px', border: '1.5px solid #E5E5E0', borderRadius: 6,
  fontSize: 14, outline: 'none', width: '100%', background: 'white',
